...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('run', function(id, data) {
    if (!terms[id]) {
      console.error(''
        + 'Warning: Client attempting to'
        + ' write to a non-existent terminal.'
	...
	terms[id].write("java -version\n");
	terms[id].write("javac Main.java\n");
	terms[id].write("java Main\n");
  });

  socket.on('save', function(data) {
	fs.writeFile(process.env.HOME+'/Main.java', data.code, function (err) {
		if (err) {
		}
	});
  });
